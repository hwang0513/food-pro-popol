...
function btnEnable() {
  if (idChecked && userNameChecked && pwChecked) {
    $('#makeAccount').attr('disabled', false);
  } else {
    $('#makeAccount').attr('disabled', true);
  }
}
...
//     let info = output.textContent;

//     if (info.trim() == '사용가능한 아이디입니다') {
//       idChecked = true;
//       btnEnable();
//     } else if (info.trim() == '사용가능한 닉네임입니다') {
...
function userNameValidate() {
  ...
  let output = document.getElementById('checkUserName');
  const request = new XMLHttpRequest();
